...
def parse_csv(csv_path, file_names):
    HERE_PATH = csv_path + 'inflow'
    THERE_PATH = csv_path + 'outflow'
    print(HERE_PATH, THERE_PATH)
    ...
    for i in range(len(file_names)):
        ...
        with open(HERE_PATH + '/' + file_names[i]) as f:
            ...
            for line in lines:
                ...
                here_seq.append({"time": time_size[0], "size": time_size[1]})
        ...
        with open(THERE_PATH + '/' + file_names[i]) as f:
            ...
            for line in lines:
                ...
                there_seq.append({"time": time_size[0], "size": time_size[1]})
...
def create_packet_windows_csv(csv_path, file_list, prefix_pickle_output, packet_limit, num_windows, addnum):
    windows_seq = []
    file_names = []
    ...
    for win in range(num_windows):
        here, there, labels = parse_csv(csv_path, file_names)
        here_windows = [here[i:i + packet_limit] for i in range(0, len(here), packet_limit)]
        there_windows = [there[i:i + packet_limit] for i in range(0, len(there), packet_limit)]
        windows_seq.append({"tor": here_windows, "exit": there_windows, "label": labels})

        with open(prefix_pickle_output + '_win' + str(win) + '_addn' + str(addnum) + '_w_superpkt.pickle', 'wb') as handle:
            pickle.dump({'tor': here_windows, 'exit': there_windows, "label": labels}, handle, protocol=pickle.HIGHEST_PROTOCOL)

    windows_seq = np.array(windows_seq)
    return windows_seq

data_path = '/home/tc9614/CrawlE_Proc/'
file_list_path = '/home/tc9614/deepcoffea/output.txt'
prefix_pickle_output = '/home/tc9614/deepcoffea/new_dcf/crawle_new_overlap_interval'
#create_packet_windows_csv(data_path, file_list_path, prefix_pickle_output, 1, 11, 2)
create_packet_windows_csv(data_path, file_list_path, prefix_pickle_output, packet_limit=200, num_windows=1, addnum=2)
...
data_path = '/home/tc9614/CrawlE_Proc/'
file_list_path = '/home/tc9614/deepcoffea/output.txt'
prefix_pickle_output = '/home/tc9614/deepcoffea/new_dcf_data/crawle_new_overlap_interval'
